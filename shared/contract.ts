...
/**
 * 模型的**思考过程**（推理模型才有的 `reasoning_content`），走 CUSTOM。
 *
 * 为什么值得单独一条通道：本地/推理模型的首答可能要一两分钟，而这段时间里
 * **正文一个字都还没生成** —— 全花在思考上。不把它转出来的话，界面上就只有一个
 * 转圈动画，用户分不清"它在想"还是"它挂了"。
 *
 * 它不是工具、不是画布状态，所以既不进 `state` 也不占 tool call ——
 * 和 `EVT_POINT_AT` 同一类：**过程性事实**，只影响这一帧之后看到什么。
 * 客户端按 `phase`（第几次模型调用）聚合成一条，正文一开始就自动收起。
 *
 * 载荷：`{ delta, phase }` —— `delta` 是这半截增量，`phase` 从 0 开始。
 */
export const EVT_REASONING = 'ice/reasoning';

/**
 * 「锚定」：这张卡片**关联工艺图上的哪个单元**（`{ value, label? }`）。
 *
 * 为什么要有它：卡片浮在工艺图上面之后，"卡片与图有没有业务关系"就变成了肉眼可见的问题 ——
 * 一张"各渠道月度销量"浮在污水处理工艺流程图上，看着就是硬凑的。
 * 锚定把关系**写进数据**：卡片知道自己说的是图上的哪个东西，于是可以在浮起来的同时
 * 把那个单元**高亮 + 镜头跟过去**（复用「指着讲」那条现成的通路），
 * 表单提交后也能回到那个单元上给一次反馈。
 *
 * 与 `ice/point-at` 的区别是**语义而不是实现**：`point-at` 是"讲到这里"（瞬时的演示动作），
 * `anchor` 是"这张卡片说的是它"（在卡片存在期间持续成立的关系）。
 * 走 CUSTOM 而不是进 state：它是"当前这张卡片"的属性，而卡片本身就由 state 表达。
 */
export const EVT_ANCHOR = 'ice/anchor';
