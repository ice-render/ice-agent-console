...
class AgentConsolePage {
  // ---------------------------------------------------------------------------
  // 常量（**静态的都排在最前**，见 AGENTS「成员顺序」：
  // static 常量 → 实例字段 → 构造函数 → 方法；这里只挪位置，语义没动）
  // ---------------------------------------------------------------------------
  ...
  constructor() {
    ...
    // **主题必须在任何组件构造之前装**：本工程的约定是"启动即定死"（`?theme=` 决定）。
    // 放这儿而不是放进 `main.ts`：这里就是"第一个会造组件的地方"的上游。
    // （库本身 1.15 起支持热切换 —— 样式槽里是主题引用，paint 时解析；定死是产品选择。）
    installTheme();
    ...
    (globalThis as any).__iceAgentConsole = {
       ...
       * （78 个符号、100 段管线、`validateWater()` 零问题）。只数像素证明不了数量对。
       */
