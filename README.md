...
| 看看出水 COD 的趋势 | 主链路：文字流式 → 参数流式拼装 → 绘图区切到图表 → **指着 9-15 那个尖峰讲** | [图表](docs/images/chart.png) |
| 看看出水实时流量 | `STATE_DELTA` → `appendData` 快路径，同一张图逐拍长数据（出水计量井 FIT-101） | [流式追加](docs/images/streaming.png) |
| 给进水泵下发指令 | **人机回环**：中断 → 绘图区切成表单 → 填完提交 → 带 `resume` 开新 run | [表单卡](docs/images/form-card.png) |
| 调整加药量 | **加药与工艺参数调整单**：13 个字段按单据语义排，覆盖 DSL 0.3.0 新接的 9 个字段类型 | [加药单](docs/images/showcase.png) |
| 故意画错 | **自修复回路**：坏 DSL → 诊断回灌 → agent 自动吐修正版 | [自修复](docs/images/self-repair.png) |
| 出水要达到什么标准 | 兜底：不画图，只回文字（**绘图区保持原样**，不是清空） | — |
...
另外还有一张**不中断**的表单：`调整加药量` 推的是「加药与工艺参数调整单」，
提交后走的是普通消息通道（客户端把值拼成 `（已提交表单：…）` 发回来），
脚本侧给一张业务回执（含每班折算量）。两条路对照着看，能看出
"这一轮要不要停下来等你"是 agent 的选择，不是框架限死的。
...
#### 3.2.1 慢模型下的三件事：等待 / 流式 / Markdown

接上自己的模型之后暴露的第一个问题不是功能，是**观感**：本地跑推理模型时首答要一两分钟，
而原来的实现要等整包回复才吐第一帧 —— 用户看到的就是"我发了句话，然后什么都没有"。
三条改动合起来解决它，每一条都对应一个具体的失败形态：

| 做什么 | 为什么 | 关键约束 |
|---|---|---|
| **「正在思考」那一行** | run 在跑、助手侧还什么都没有时，至少要让人知道"它在想" | 判据是**本轮**（最后一条用户消息之后）有没有内容，**不是**扫整个 thread —— 后者只要历史里出现过工具卡就永远为真，新一轮再也不显示（这条是真实回放抓到的，单测状态干净时看不出来） |
| **流式收消息** | 推理模型的思考往往比正文还长；等整包等于把最长的等待藏起来 | `chatStream` 把 SSE 增量**立刻**转成 AG-UI 事件；推理内容走 `CUSTOM ice/reasoning`（按 `phase` 聚合），正文走 `TEXT_MESSAGE_CONTENT`；服务端若忽略 `stream:true` 按 `content-type` 退回整包解析 |
| **Markdown 渲染** | 模型的回答天然带小标题 / 列表 / 表格 / 代码块，整段当纯文本写进去就是一堆 `**` 和 `|` | 用 `markdown-it` 拿 token 流**自己建 DOM 节点**，全程没有 `innerHTML`（模型输出是不可信输入）；`href` 只放行 `http(s)`/`mailto` 并加 `rel="noopener"`；图片不自动加载，退化成可点文字 |

三种状态的交接是**自动**的，不需要谁去手动切：`正在思考` →（思考或正文一到）→ 收起；
思考块在正文开始的那一刻**自动折叠**（想看细节还能点开）；正文每来一个 delta 就整段重渲染 ——
半截的 `**加粗` 或没闭合的代码围栏都会退化成普通段落，不会抛错、不会丢字。

实测（本机 120 上跑的 `qwopus-coder`，真实浏览器）：

| 时刻 | 画面 |
|---|---|
| 0.2s | 「正在思考」出现 |
| 0.6s | 思考过程开始流入，等待提示让位 |
| 0.6–6.8s | 思考从 3 字涨到 203 字（折叠块里实时滚动） |
| 7.1s | 正文开始逐字出现，Markdown 同步重排（小标题 / 粗体 / 表格） |

依赖只加了两个，都是按"必须"来的：`markdown-it`（**客户端**运行时依赖 —— 服务端仍然只有
`@ag-ui/core`）、`jest-environment-jsdom`（devDependency，只为测 DOM 渲染那一层）。
...
| 工具名 | 图层 | 画布 | 生命周期 |
|---|---|---|---|
| `render_diagram` | `diagram` | 一块（`ice-entity-designer`）。**kind-first** 图 DSL，目前一种 kind = `water-process` | **boot 时建，永不销毁、永不隐藏** |
...
**工艺图永远在，图表 / 表单是浮在它上面的卡片。** 这是 2026-09-18 改的：
之前是"一次只显示一层"，切到图表就把工艺图 `hidden` 掉 —— 屏幕变成"一片空白 +
一张孤零零的图表"，第一反应是"我那张工艺图被清掉了"（其实它还在 DOM 里，只是没显示）。
现在 chart / form 有背景 / 圆角 / 阴影，压在工艺图上，下面垫一层接点击的遮罩，
右上角有「✕」——**点 ✕ 或点卡片外面的空白都能收起**，回到只有工艺图的状态。

由此带来的两条约束（都踩过）：

- **表单层的层根是整屏透明的**（卡片是里面那块 `.stage-form`）：层根要 `pointer-events:none`、
  卡片 `auto`，否则"点空白收起"永远不生效，而且不报错；
- **卡片装指针屏蔽时要放行画布上的事件**（`shieldFromCanvas(el, { passCanvasEvents: true })`）：
  `ice-chart` 的悬停 / 框选靠 window 级广播 + `isOverCanvas()` 过滤，一刀切会把图表交互弄哑。

代价说清楚：卡片期间**底下的工艺图不能拖拽缩放**（遮罩吃掉了指针），工艺图画布上那几个按钮
（比如「看整张图」）也被压住 —— 先收起卡片再用。

#### 卡片必须"锚定"到图上的某个单元

浮层做出来之后暴露的下一个问题是**业务关联**：一张"各渠道月度销量"浮在污水处理工艺流程图上，
看着就是硬凑的 —— 把图遮住，那张卡片照样说得通，说明它根本没绑在图上。

所以每张卡片带一个**锚定**（`anchor`，见 `shared/contract.ts` 的 `EVT_ANCHOR`）：

| | 说明 |
|---|---|
| 它是什么 | `{ value, label? }` —— `value` 是单元 id 或位号（如 `codAnalyzer` / `AIT-106`） |
| 表现 | 卡片浮起来的同时，**图上那个单元被高亮、镜头跟过去**（复用「指着讲」那条通路） |
| 闭环 | 表单提交后再闪一次那个单元 —— 卡片上的"已提交"是界面内的事，这一下才说明"落到图上了" |
| 从哪来 | 剧本里写死（`scenarios.ts`）；模型也可以给（工具参数顶层加 `anchor`，**渲染前会被摘掉**，见下） |

三条纪律：

- **题材与图上对象相关**：图表画的是某个单元的运行数据（出水 COD 趋势 → 在线监测仪 AIT-106）、
  表单是对某个单元的操作（给进水泵 P-101 下指令）。**判据是把图遮住，卡片还说不说得通**；
- **模型给的 anchor 要摘干净**：它写在工具参数顶层，而那一层就是图表 DSL ——
  DSL 校验只认自己的字段，多一个键会被判不合法、然后走成"自修复"，把一张好卡片修没
  （`buildLlmPlan` 里 `delete rest.anchor`，单测钉着）；
- **锚不上就不锚**：模型给了图里没有的位号时，`StageView.anchor()` 返回 false，
  不改镜头、不报错，卡片照常显示 —— 宁可少一层呼应，也不要指错地方。

图层之间**不并排**（各自一块画布、各自一个 ICE 实例），所以用不上
`linkViewport` / `setInputPassthrough` / `composeLayersToCanvas` —— 那些只在层与层重叠时有意义。

```
#stage  position:fixed; inset:0
├── .stage-layer[data-kind="diagram"]        ← boot 建，永不销毁、永不隐藏（主视图）
│     └── <canvas>   ICE 实例 ①，WaterProcessDesigner
├── .stage-scrim                             ← 按需：接住点击（点空白 = 收起卡片）
├── .stage-layer[data-kind="chart"]          ← 按需：浮在工艺图上的卡片
│     ├── .stage-chart  <canvas>   ICE 实例 ②（ice-chart 自己 new 的）
│     └── .stage-widget <canvas>   ICE 实例 ③（控件条，卡片底部）
└── .stage-layer[data-kind="form"]           ← 按需：同样浮着（层根整屏透明）
