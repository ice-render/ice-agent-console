...
类里的成员按这个顺序排 —— 就是棘轮里那个正则 `S*T*F*C*(A|M)*`：
...
全家族的口径一致 —— smart-water 的 12 个页面、各仓示例页都是这个形状，它们的静态成员
正好是"这个页面类型的版面几何/常量"，本来就要在建实例之前用。
...
1. **`src/domain/theme.ts`** —— 主题的唯一事实来源（家族品牌色 + 浅/深两套 token 补丁 + 局部 token），
   再由 `applyThemeToCss()` 写成 CSS 变量、`applyThemeToEngine()` 打给每个 ICE 实例；
2. **组件库的 token**（画布那半的组件样式）。

别处写死的十六进制色值就是"第三套来源"：不会跟主题走，而且**不会有任何测试变红**
（2026-09-17 在 `ice-smart-water` 就是这么漏了 12 处界面色，靠人工 grep 才挖出来）。
棘轮：`tests/colorSource.test.ts`（剥注释后扫 `src/**/*.ts`；豁免只有两个文件、且都写了理由：
主题定义本身、"指着讲"的语义高亮黄）。

> 画布那半的取色请写成**主题引用**（`token('ui.colors.x')`）而不是构造期取值 ——
> 引用是 paint 时解析的，换主题（以及每个 ICE 实例各自的主题）才跟得上。
